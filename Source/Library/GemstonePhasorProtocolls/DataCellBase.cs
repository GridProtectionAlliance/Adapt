    ...
    public abstract class DataCellBase : ChannelCellBase, IDataCell
    {
        ...
        public uint CommonStatusFlags
        {
            get
            {
                ...
                if (!DataIsValid)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataIsValid;

                if (!SynchronizationIsValid)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.SynchronizationIsValid;

                if (DataSortingType != DataSortingType.ByTimestamp)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataSortingType;

                if (DeviceError)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.DeviceError;

                if (m_isDiscarded)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataDiscarded;

                if (DataModified)
                    commonFlags |= (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataModified;
            ...
            set
            {
                ...
                DataIsValid = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataIsValid) == 0;
                SynchronizationIsValid = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.SynchronizationIsValid) == 0;
                DataSortingType = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataSortingType) == 0 ? DataSortingType.ByTimestamp : DataSortingType.ByArrival;
                DeviceError = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.DeviceError) > 0;
                m_isDiscarded = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataDiscarded) > 0;
                DataModified = (value & (uint)GemstonePhasorProtocolls.CommonStatusFlags.DataModified) > 0;
            }
        }
