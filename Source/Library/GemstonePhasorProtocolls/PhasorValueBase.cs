...
namespace GemstonePhasorProtocolls
{
    ...
    public abstract class PhasorValueBase : ChannelValueBase<IPhasorDefinition>, IPhasorValue
    {
            ...
            : base(info, context)
        {
            ...
            m_phasor = new Complex(info.GetDouble("real"), info.GetDouble("imaginary")) ;
        }
        ...
        {
            get => m_phasor.Phase;
            set => m_phasor = Complex.FromPolarCoordinates(m_phasor.Magnitude, value);
        }
        ...
        public virtual double Magnitude
        {
            ...
            set => m_phasor = Complex.FromPolarCoordinates(value, m_phasor.Phase);
        }
        ...
        {
            get => m_phasor.Real;
            set => m_phasor = new Complex(value, m_phasor.Imaginary);
        }
        ...
        public virtual double Imaginary
        {
            ...
            set => m_phasor = new Complex(m_phasor.Real,value);
        }
        ...
        public virtual int UnscaledReal
        {
            ...
            set => m_phasor = new Complex(value * Definition.ConversionFactor, m_phasor.Imaginary);
        }
        ...
        public virtual int UnscaledImaginary
        {
            ...
            set => m_phasor = new Complex(m_phasor.Real, value * Definition.ConversionFactor);
        }
        ...
        /// </summary>
        public override bool IsEmpty => double.IsNaN(m_phasor.Real) && double.IsNaN(m_phasor.Imaginary);
                    ...
                    else
                    {
                        if (DataFormat == DataFormat.FixedInteger)
                        {
                            ...
                            if (AngleFormat == AngleFormat.Radians)
                                BigEndian.CopyBytes((short)(m_phasor.Phase * 10000.0D), buffer, 2);
                            else
                                BigEndian.CopyBytes((short)(new Gemstone.Units.Angle(m_phasor.Phase).ToDegrees() * 10000.0D), buffer, 2);
                        }
                        else
                        {
                            ...
                            if (AngleFormat == AngleFormat.Radians)
                                BigEndian.CopyBytes((float)m_phasor.Phase, buffer, 4);
                            else
                                BigEndian.CopyBytes((float)(new Gemstone.Units.Angle(m_phasor.Phase)).ToDegrees(), buffer, 4);
                        }
                    }
                }
        ...
        protected override int ParseBodyImage(byte[] buffer, int startIndex, int length)
        {
            ...
            if (DataFormat == DataFormat.FixedInteger)
            {
                ...
                else
                {
                    ...
                    double magnitude = BigEndian.ToUInt16(buffer, startIndex) * Definition.ConversionFactor;
                    Angle phase;

                    if (AngleFormat == AngleFormat.Radians)
                        phase = BigEndian.ToInt16(buffer, startIndex + 2) / 10000.0D;
                    else
                        phase = Angle.FromDegrees(BigEndian.ToInt16(buffer, startIndex + 2) / 10000.0D);

                    m_phasor = Complex.FromPolarCoordinates(magnitude, phase);
                }
            ...
            if (CoordinateFormat == CoordinateFormat.Rectangular)
            {
                ...
                m_phasor = new Complex(BigEndian.ToSingle(buffer, startIndex), BigEndian.ToSingle(buffer, startIndex + 4));
            }
            else
            {
                // Parse from single-precision floating-point, polar
                double magnitude = BigEndian.ToUInt16(buffer, startIndex);
                Angle phase;
                
                if (AngleFormat == AngleFormat.Radians)
                    phase = BigEndian.ToSingle(buffer, startIndex + 4);
                else
                    phase = Angle.FromDegrees(BigEndian.ToSingle(buffer, startIndex + 4));
